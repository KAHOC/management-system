...
public class Tb_studentServlet extends HttpServlet {
	...
	private void Studel(HttpServletRequest request, HttpServletResponse response) 
		...
		Tb_studentDao sdao = new Tb_studentDao();
		String id = request.getParameter("id"); 
		String[] tid = id.split(",");
		for (String s : tid) {
			sdao.deleteStuInfo(s);  //循环删除
		}
	...
	private void Stulist(HttpServletRequest request,
		...
		slist=sdao.StudentList();
		request.setAttribute("slist", slist); 
		request.getRequestDispatcher("Student.jsp").forward(request, response);//请求转发到jsp页面
	}
